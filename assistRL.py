...
class Params:
    EPOCH_SIZE_IN_EACH_TRAJECTORY = 200  # How many epochs we want to pack when transferring parameters
    EPOCH_SIZE_IN_SHARE = 20    # How many parameters will be shared during each assistance
    ALPHA = 5e-4       # learning rate
    EPISODE_SIZE_IN_EACH_EPOCH = 500   # how many episodes we want to pack into an epoch
...
class PolicyGradient:
    def __init__(self, environment: str = "CartPole", use_cuda: bool = False, height_level=0.25,LEG_SPRING_TORQUE=40, map_prob=1, map_n=10, msg1=None, msg2=None):
        ...
        self.opt = optim.SGD(params=self.agent.parameters(), lr=self.ALPHA)
        # the total_rewards record mean rewards in the latest 100 episodes
        self.total_rewards_each_episode = []
    ...
    def self_train(self):
        ...
        loss_in_accumulate_epoch = deque([], maxlen=100)
        if self.record:
            self.env = wrappers.Monitor(self.env, './arl_video/' + self.msg1 + '/'+ self.msg2, video_callable=lambda episode_id: True,
                                   force=True)
        while epoch < self.EPOCH_SIZE_IN_EACH_TRAJECTORY:
            loss, reward_epoch_mean = self.play_epoch_use_agent(self)
            loss_in_accumulate_epoch.append(loss.data.cpu().numpy())
    ...
    def play_epoch_use_agent(self, provider, test=False):
        ...
        loss, entropy = self.calculate_loss(epoch_logits=epoch_logits,
                                            weighted_log_probs=epoch_weighted_log_probs)
        # if the epoch is over - we have epoch trajectories to perform the policy gradient
        return loss, np.mean(rewards_epoch)
